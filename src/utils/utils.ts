...
const implementHierarchy = (necessaryPermission:string): boolean => {
	...
	case 'admin':
		return ['sysAdmin', 'admin'].includes(atob(localStorage.getItem('userType') || ''))
	case 'hub':
		return ['sysAdmin', 'admin', 'hub'].includes(atob(localStorage.getItem('userType') || ''))
	case 'store':
		return ['sysAdmin', 'admin', 'hub', 'store'].includes(atob(localStorage.getItem('userType') || ''))
	case 'seller':
		return ['sysAdmin', 'admin', 'hub', 'store', 'seller'].includes(atob(localStorage.getItem('userType') || ''))
	default:
		return false
	}
}
...
class ShowDialog {
	...
	static showSendMessagePopUp = (title:string, description:string, message = ''): void => {
		Dialog.create({
...
class AddressUtils {
	private static mixAddresses = (addressReceiver: Address, searchedAddress: Address): void => {
		...
		addressReceiver.neighborhood = addressReceiver.neighborhood || searchedAddress.neighborhood
		...
		addressReceiver.complement = addressReceiver.complement || searchedAddress.complement
		addressReceiver.ibgeCode = addressReceiver.ibgeCode || searchedAddress.ibgeCode
	...
	static getAddress = async (cep: ZipCode, addressReceiver: Address): Promise<void> => {
		await ShowLoading.show(`Buscando endereço para o CEP ${cep.getCodeFormatted()}...`)
		try {
			const result = await useAddressStore().fetchCep(cep.getCodeWithoutSymbols())
