  ...
  <q-layout>
    <q-page class="column">
      <q-select
        ...
        map-options
      />
      <div class="row q-gutter-md items-center" v-if="userType == 'establishmentId'">
        <q-select
      ...
      <div class="row q-gutter-md items-center" v-if="userType == 'userId'">
        <q-select
          v-model="searchValueBy"
          filled
          dense
...
const validateSearchType = () => {
  ...
  } else if (searchValueBy.value == 'name' || searchValueBy.value == 'tradeName') {
    enable.value = Validator.isValidName(searchValue.value);
  }else {
    enable.value = false;
  }
};
