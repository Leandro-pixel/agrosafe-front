  ...
  <div class="q-pa-md">
    <q-stepper v-model="step" vertical color="primary" animated transition-duration="500" class="default-box-shadow big-border-radius">
      <q-step :name="1" title="Dados" icon="settings" :done="step > 1" caption="Obrigatório" class="column justify-content">
        <p class="text-h6 text-bold">Informações do Polo</p>
        <div>
          <q-input dense outlined v-model.trim="fullName" label="Nome completo" lazy-rules class="half-width" :rules="[(val:string) => Validator.isValidName(val) && Validator.hasMultipleWords(val) || 'Campo obrigatório']"  />
          <q-input dense outlined v-model.trim="fantasyName" label="Nome fantasia" lazy-rules class="half-width" :rules="[(val:string) => Validator.isValidName(val) || 'Campo obrigatório']" />
      ...
      <q-step :name="3" title="Criar Polo" icon="check">
...
const submit = async () => {
	try {
		...
		const response = await hubStore.createHub(hub)
